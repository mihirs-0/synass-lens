#!/usr/bin/env python3
"""HIERARCHICAL DISAMBIGUATION v2: K1=20, K2=10
=================================================

Redesigned for a visible staircase. Key changes from v1 (K1=5, K2=4):
  - K1=20, K2=10 → large information asymmetry (log(20)/log(10) = 1.30)
  - n_b=1000 → D_second = 1000×10 = 10,000 → second plateau tau ≈ 1,500-1,850 steps
  - Total K=200, D=200,000
  - No unique-first-char constraint (K=200 > 36 chars)
  - z1 uses first 20 chars of vocab, z2 uses next 10

Prediction:
  Phase 0: log(200) ≈ 5.30  (ignoring both z1, z2)
  Phase 1: log(10) ≈ 2.30   (using z1 only)
  Phase 2: ≈ 0              (using both z1, z2)

Sequence format unchanged: [BOS, B(6), SEP, z1_char, z2_char, SEP, A(4), EOS]
"""
...
N_B = 1000          # unique B groups
D = N_B * K         # total examples = 200,000

LR = 1e-3
BATCH_SIZE = 128
MAX_STEPS = 100_000
EVAL_EVERY = 100        # coarser eval (K=200 makes candidate_eval expensive)
...
OUTPUT_DIR = Path("outputs/hierarchical_test_v2")

LOG_K = math.log(K)       # 5.298
LOG_K2 = math.log(K2)     # 2.303
LOG_K1 = math.log(K1)     # 2.996
...
def generate_hierarchical_data():
    ...
    Z-encoding: z = z1_char + z2_char where
      z1_char in first 20 chars of vocab   (K1=20 cluster selectors)
    ...
    z1_chars = list(VOCAB_CHARS[:K1])             # a-t (20 chars)
    z2_chars = list(VOCAB_CHARS[K1:K1 + K2])      # u-3 (10 chars)
    ...
    for group_idx in range(N_B):
        ...
        # K=200 unique A-targets (NO unique-first-char constraint)
        a_list = []
        for _ in range(K):
...
def run():
    ...
    print("=" * 60)
    print("HIERARCHICAL DISAMBIGUATION v2")
    print("=" * 60)
    ...
    cfg = OmegaConf.create({
        "experiment": {"name": "hierarchical_v2", "seed": SEED},
        "data": {
    ...
    # Reduce candidate_eval samples since K=200 makes each expensive
    CAND_EVAL_N = 16
...
def make_figure(history):
    ...
    ax1.set_title(f"Hierarchical Disambiguation v2: "
                  f"K1={K1} clusters × K2={K2} targets (D={D:,})",
                  fontsize=14)
    ax1.legend(fontsize=11, loc="upper right")
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(bottom=-0.1, top=LOG_K + 0.5)

    # Panel B: Z-usage diagnostics
    ...
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / "hierarchical_staircase_v2.png", dpi=150)
    plt.close(fig)
    print(f"\nFigure saved: {OUTPUT_DIR / 'hierarchical_staircase_v2.png'}")
